...
# متغيرات بيئية / أسرار
ENV_PATTERN_ENTRIES: Tuple[Tuple[re.Pattern[str], bool], ...] = (
    (re.compile(r"os\.getenv\(['\"]([A-Z0-9_]+)['\"]\)"), False),
    (re.compile(r"os\.environ\[['\"]([A-Z0-9_]+)['\"]\]"), False),
    (re.compile(r"process\.env\.([A-Z0-9_]+)"), False),
    (re.compile(r"\$\{([A-Z0-9_]+)\}"), False),
    (re.compile(r"\$\{\{\s*secrets\.([A-Za-z0-9_]+)\s*\}\}"), True),
)
...
def analyze_repo(repo_path: str) -> Dict[str, Any]:
    ...
    for filepath in files:
        ...
        for pattern, is_github_secret in ENV_PATTERN_ENTRIES:
