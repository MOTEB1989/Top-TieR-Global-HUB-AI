...
def main():
    """الدالة الرئيسية"""
    print("\n" + "="*60)
    print("🔑 فحص جميع مفاتيح API والإعدادات")
    print("="*60 + "\n")
    ...
    print(f"  ✅ صالحة: {valid_keys}")
    print(f"  ❌ مفقودة: {missing_keys}")
    print(f"  ⚠️  قيم افتراضية: {placeholder_keys}")
    ...
    if missing_keys > 0 or placeholder_keys > 0:
        ...
        if placeholder_keys > 0:
            ...
            print("     - افتح ملف .env")
            print("     - استبدل القيم التي تحتوي على PASTE_ أو ${{")
            print("     - أضف المفاتيح الفعلية من الخدمات المعنية")
    ...
    if percentage >= 80:
        ...
        print("⚠️  يحتاج إلى بعض التحسينات")
        return 0
    else:
        print("❌ يحتاج إلى إعداد إضافي")
        return 1
